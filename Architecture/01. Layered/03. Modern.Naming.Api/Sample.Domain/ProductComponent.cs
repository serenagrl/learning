...
namespace Sample.Business
{
    ...
    {
        // Note: Generally, business components only have single implementation and don't need
        //       interfaces but if your environment needs to support multiple different 
        //       implementations, then it is best to create an interface for your business
        //       components i.e. IProductComponent.

        private readonly ILogger<ProductComponent> _logger;
        private readonly IConfiguration _config;

        private readonly ProductRepository _productDAC;

        public ProductComponent(ILogger<ProductComponent> logger,
            IConfiguration config, ProductRepository productDAC 
            )
        {
            this._logger = logger;
            this._config = config;
