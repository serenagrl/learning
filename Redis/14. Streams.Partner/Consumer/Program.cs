    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            do
            {
                ...
                {
                    Console.Write("Enter Partner Consumer name: ");
                    partnerName = Console.ReadLine();

                    if (!string.IsNullOrWhiteSpace(partnerName))
                    {
                        // Subscribe to partner's channel.
                        sub.Subscribe($"Telemetry.{partnerName}", (channel, msg) =>
                        {
                            // Claim the pending message.
                            var entry = db.StreamClaim(KEY_NAME, GROUP_NAME, consumerName, 0, new[] { msg });
                        });
                    }
                ...
                Console.Write($"{consumerName}-{partnerName}$ [Enter] to read stream, [P]ending messages, [X] to exit: ");
            ...
            foreach (var entry in entries)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        sub.Publish($"Telemetry.{consumerName}", entry.Id);
